...
'''
run makemore.py -i names_short.txt -o names --type mlp --max-steps 10000
run makemore.py -i names_short.txt -o names --type mlp --sample-only
...
# this is decorator
# a decorator takes in a function or a class (which is a more expressive function)
# and adds to it without changing it's core purpose.
# like a decorator can make a room look good but doesn't change
# it's core purpose.
# in this case the decorator means that several core methods are automatically generated.
# including the __init__ method which is why there is no constructor but later object
...
class RNNCell(nn.Module):
    ...
    # normally calculate the hidden layer twice and add them once from last hidden and once from current token
...
class RNN(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # decode the outputs
        hidden = torch.stack(hiddens, 1) # (b, t, n_embd2) - batch, timesteps,num units in hidden layer - ultimate extract the final hidden state at the final timestep for each batch
...
# which is fine in this case as count has range [0,inf)
...
if __name__ == '__main__':
    ...
    # init model configuration via new dataclass class
    # essentially create an object via the constructor.
