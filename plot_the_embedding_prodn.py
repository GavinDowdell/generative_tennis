...
def plot_embedding(dataset,labels,out_path='.',num_cluster=8,transparent=False,tsne=False):
    ...
    fig.update_layout(height=1000,width=1500,title_text='Shot Embedding')
    plotly.offline.plot(fig,filename=os.path.join(out_path,'pca2.html'))
    #fig.show()
    ...
    if transparent:
        fig.update_traces(textfont_color='rgba(0,0,0,0)')
    fig.update_layout(height=1000,width=1500,title_text='Shot Embedding')
    plotly.offline.plot(fig,filename=os.path.join(out_path,'tsne2.html'))
    #fig.show()
